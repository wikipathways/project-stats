...
def get_app_citations_from_file_as_dict(queryfile):
    ...
    X is a human readable nickname for the publication, ID is the pubmed id of the
    publication 

    :param queryfile: tab delimited file containing names and
                      PMIDs of publications
    :type queryfile: str
    :return: dict where key is name and value is citation
    :rtype: dict
    """
    df = pandas.read_csv(queryfile, delimiter='\t', header=0)
    citation_dict = dict()
    for index, row in df.iterrows():
        citation_dict[row['name']] = (row['citation'])
...
def download_medline_for_publications(urlprefix=None, outfile=None, toolargs=None, the_ids=None,
    ...
    # get medline file for publication
...
def write_app_report_csv(outfile=None, citation_dict=None, cited_pubs=None):
    """
    Writes out a CSV file with information about the publication

    :param outfile: output CSV file
    :param citation_dict: dict where key is name
                          and value is citation id
    :type citation_dict: dict
    :param cited_pubs: dict where key is name
    ...
    with open(outfile, 'w') as f:
        ...
        for key in citation_dict.keys():
            row = {'Name': key,
                   ...
                   'Origin': ' '.join(citation_dict[key][1]['origin']),
                   'Journal': ' '.join(citation_dict[key][1]['journal']),
                   'PublishDate': ' '.join(citation_dict[key][1]['publishdate'])}
            writer.writerow(row)
    return None
...
def plot_journal_summary(inputfile=None, tool_name='My Project',
...
def plot_grant_summary(inputfile=None, tool_name='My Project',
                       outfile=None, top_count=15):
    """
...
def plot_publishdate_summary(inputfile=None, tool_name='My Project',
                             outfile=None):
    """
...
def main(args):
    ...
    Where 'X' is a nickname, 'ID' is 
    the pubmed id of publication
    ...
    data/
        - For each name contains a
          medline file of the publication,
          a json file containing the list of papers
          citing the publication, and a medline
          file for all publications citing the
          publication. This is kept for caching
          purposes.
    ...
    cited_publications_grants.csv
        - CSV containing counts of grants
          ...
          papers in pmids.txt. Logic has been
    ...
    cited_publications_per_year.csv
        - CSV containing counts of publications
          ...
          citing papers in pmids.txt. This is
          calculated using the 'unique medline file'
    ...
    top_cited_publications_journal.svg
        - Bar chart figure using top 15 publication venues
          for publications that cited papers in pmids.txt.
    ...
    toolargs = '&tool=PubStats&email=' + theargs.email
    ...
    for key in tqdm(citation_dict.keys()):
        ...
        if not os.path.isfile(medlinefile):
            download_medline_for_publications(urlprefix=urlprefix,
                                              outfile=medlinefile,
                                              toolargs=toolargs,
                                              the_ids=citation_dict[key])

        # get id of publications citing the query publication
        ...
        # get count of publications citing the query publication
        ...
        # get information from medline about the query publication
        article_dict = get_article_info_from_medline(medlinefile=medlinefile)

        # add article information to citation_dict tuple
        citation_dict[key] = (citation_dict[key], 
                              article_dict)

    # write summary report of the query publications
    write_app_report_csv(outfile=os.path.join(outdir, 'summary_report.csv'),
    ...
    with open(os.path.join(outdir, 'summary.txt'), 'w') as f:
        ...
        f.write('Number of publications: ' + str(len(citation_dict.keys())) + '\n')
